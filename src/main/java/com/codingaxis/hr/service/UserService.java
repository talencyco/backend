...
import com.codingaxis.hr.domain.Authority;
import com.codingaxis.hr.domain.User;
import com.codingaxis.hr.security.AuthoritiesConstants;
import com.codingaxis.hr.security.BCryptPasswordHasher;
...
public class UserService {
  ...
  public void changePassword(String login, String currentClearTextPassword, String newPassword) {
    ...
    User.findOneByLogin(login).ifPresent(user -> {
      ...
      if (!this.passwordHasher.checkPassword(currentClearTextPassword, currentEncryptedPassword)) {
  ...
  public Optional<User> completePasswordReset(String newPassword, String key) {
    ...
    return User.findOneByResetKey(key).filter(user -> user.resetDate.isAfter(Instant.now().minusSeconds(86400)))
        .map(user -> {
          user.password = this.passwordHasher.hash(newPassword);
          user.resetKey = null;
          user.resetDate = null;
  ...
  public Optional<User> requestPasswordReset(String mail) {
    ...
    return User.findOneByEmailIgnoreCase(mail).filter(user -> user.activated).map(user -> {
  ...
  public User registerUser(UserDTO userDTO, String password) {
    ...
    newUser.password = this.passwordHasher.hash(password);
  ...
  public User createUser(UserDTO userDTO) {
    ...
    user.password = this.passwordHasher.hash(RandomUtil.generatePassword());
    user.resetKey = RandomUtil.generateResetKey();
    user.resetDate = Instant.now();
    user.activated = true;
    if (userDTO.authorities != null) {
      user.authorities = userDTO.authorities.stream()
          .map(authority -> Authority.<Authority> findByIdOptional(authority)).filter(Optional::isPresent)
  ...
  public Optional<UserDTO> updateUser(UserDTO userDTO) {
    ...
    return User.<User> findByIdOptional(userDTO.id).map(user -> {
      user.login = userDTO.login.toLowerCase();
      user.firstName = userDTO.firstName;
      user.lastName = userDTO.lastName;
      if (userDTO.email != null) {
      ...
      userDTO.authorities.stream().map(authority -> Authority.<Authority> findByIdOptional(authority))
   ...
   */
  public void updateUser(String login, String firstName, String lastName, String email, String langKey,
      String imageUrl) {
  ...
  public List<UserDTO> getAllManagedUsers() {
    ...
    return User.findAllByLoginNot(Page.ofSize(20), Constants.ANONYMOUS_USER).stream().map(UserDTO::new)
